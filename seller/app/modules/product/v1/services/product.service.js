...
class ProductService {
        ...
        for (let product of data) {
          ...
          if (Object.keys(customizationDetails).length > 0) {
            ...
            for (const customizationGroup of customizationGroups) {
              ...
              if (customGroupsTags.find((cgt) => cgt.value === cgId)) {
                continue;
              }
          ...
          if (product.type === "customization") {
            ...
            const customizationMap = await CustomizationGroupMapping.findOne({
          ...
          if (!items[product_id]) {
            items[product_id] = {
              ...
              "@ondc/org/available_on_cod": product.availableOnCod,
              "@ondc/org/contact_details_consumer_care":
                store.supportDetails.email + ", " + store.supportDetails.mobile,
              tags: [
                {
                  code: "type",
                  list: [
                  ...
                  list: [
                    {
                      ...
                      value:
                        ...
                        product.vegNonVeg?.toLowerCase().substring(0, 3) ===
                        "veg"
                          ? "yes"
                          : "no",
                    },
    ...
    try {
      ...
      if (params.city) {
        ...
        orgs = await Organization.find({
          "storeDetails.address.area_code": { $in: cityData },
        }).lean();
      } else {
  ...
  async ondcGetForUpdate(productId) {
    try {
      ...
      product.attributes = attributeData;
      product.customizationDetails = await customizationService.mappdedData(
        product.customizationGroupId,
        { organization: product.organization }
      );
      return product;
  ...
  async update(productId, data, currentUser) {
    try {
      ...
      this.notifyItemUpdateToCore(productId, currentUser.organization);
  ...
  async notifyItemUpdateToCore(product_id, orgId) {
    const req = {
      seller: {
    ...
    console.log("[SELLER_APP_TO_CORE] item update", req);
          ...
          for (const data of groupData) {
            if (data.id === customizationDataDefaultMAX._id) {
              if (data.groups && data.groups.length > 0) {
                for (const group of data.groups) {
                  if (group.child) {
                    maxDefaultMRP = await this.maxDefaultMRP(
                      maxDefaultMRP,
                      group.child,
                      currentUser
                    );
